...
async function handleInactive(customerElement) {
    ...
    try {
        const response = await fetch('api_keys/sales.php', {
            method: 'POST',
...
// Example usage
try {
    const billDate = parseDate('2023-13-01'); // Invalid date
    console.log('Parsed Date:', billDate);
} catch (error) {
    console.error('Error:', error.message);
    showStatusMessage('Invalid date provided. Please check the input.', 'error');
}
...
async function loadData(action) {
    ...
    try {
        const response = await fetch(`api_keys/sales.php?action=${action}`);
...
function renderData(action, data) {
    ...
    container.innerHTML = data.map(row => {
        ...
        return `
        <div class="customer-card ${action === 'bill_value' ? 'expandable-bill' : ''}">
            <!-- Card Header -->
            <div class="customer-header" ${action === 'bill_value' ? `onclick="toggleBillDetails('${mobileNo}', this.parentElement)"` : ''}>
            <input type="hidden" class="CustCd" value="${CustCd}">
                ...
                ${action === 'bill_value' ? '<i class="expand-icon fas fa-chevron-down"></i>' : ''}
            </div>
    ...
    switch (action) {
        case 'customer':
            html += `<div class="customer-info">
                        ...
                        <strong style="float:right">Last Call: ${escapeHtml(row.Lst_Cltime || 'N/A')}<br></strong>
                     </div>`;
            break;
        case 'bill_value':
            html += `<div class="bill-info">
                        <strong>Bill Value:</strong> ₹${parseInt(row.bill_value || 0).toLocaleString('en-IN')}
                     </div>`;
            break;
        case 'bday':
            html += `<div class="bday-info">
                        <strong>Birthday:</strong> ${escapeHtml(row.CustDOB || 'N/A')}
                     </div>`;
            break;
        case 'visit_cnt':
            html += `<div class="visit-info">
                        <strong>Visit Count:</strong> ${escapeHtml(row.visit_count || 0)}
                     </div>`;
            break;
        case 'lst_visit':
            html += `<div class="visit-info">
                        <strong>Last Visit:</strong> ${escapeHtml(row.last_visit || 'N/A')} <br>
                        <strong>Days Ago:</strong> ${escapeHtml(row.Cnt || 'N/A')}
                     </div>`;
            break;
        default:
...
async function fetchBillDetails(mobileNo, billDetailsContainer) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost/billingsystem/api_keys/sales.php?action=bill_details&mobileNo=${encodeURIComponent(mobileNo)}`);
...
function renderBillDetails(bills, container, mobileNo, customerName) {
    if (!bills || bills.length === 0) {
        container.innerHTML = '<div class="no-bills">No bill history found</div>';
        return;
    }
    ...
    container.innerHTML = `
        <div class="bill-history">
            <h4>
                ...
                Bill History: ${escapeHtml(customerName || '')} (${mobileNo})
            </h4>
            <div class="bill-summary">
                <span>Total Bills: ${bills.length}</span>
                <span>Last Bill: ${bills[0].bill_date}</span>
            </div>
                ...
                <tbody>
                    ${bills.map(bill => `
                        <tr>
                            ...
                            <td class="text-right">${parseFloat(bill.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
...
function renderPaginatedCards(action) {
    ...
    container.innerHTML = pageData.map(row => {
        const mobileNo = escapeHtml(row.CustMobileNo || 'N/A');
        const isVerified = row.OTPVerified === 'Y';
        // const Lst_Cltime = row.Lst_Cltime ? escapeHtml(row.Lst_Cltime) : 'N/A';
        const CalledToday = row.CalledToday === 'Y';
        const currentDate = new Date().toISOString().split('T')[0]; // Get current date in YYYY-MM-DD format
        let bg_color = '';
        let text_color = '';
        
        const formattedDate = new Date(row.Lst_Cltime).toISOString().split('T')[0]; // Convert to YYYY-MM-DD format
        if (CalledToday && formattedDate === currentDate) {
        ...
        const CustCd = escapeHtml(row.CustCd || '');
        return `
        <div class="customer-card ${action === 'bill_value' ? 'expandable-bill' : ''}" style="${bg_color}${text_color}">
            <div class="customer-header" ${action === 'bill_value' ? `onclick="toggleBillDetails('${mobileNo}', this.parentElement)"` : ''}>
                <input type="hidden" class="CustCd" value="${CustCd}">
            ...
            ${action === 'bill_value' ? `<div class="bill-details" style="display:none;"></div>` : ''}
            <div class="action-buttons">
                <button class="action-btn call-btn" onclick="handleCall(this)">
                    <i class="fa-solid fa-phone"></i> Call
                </button>
    ...
    document.getElementById('search-box').addEventListener('input', function () {
        ...
        if (!q) {
            ...
            filteredCardsData = allCardsData.filter(card =>
                ...
                (card.bill_value && card.bill_value.toString().includes(q)) ||
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Toggle filter options when Lead button is clicked
    // leadBtn.addEventListener('click', function (e) {
    //     e.stopPropagation();
    //     leadFilters.style.display = leadFilters.style.display === 'block' ? 'none' : 'block';
    // });

    // Hide filters when clicking elsewhere
